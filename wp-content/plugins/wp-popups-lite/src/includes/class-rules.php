	...
	public static function rule_match_woocommerce( $rule ) {
		...
		switch ( $rule['rule'] ) {
			...
			case 'woo_is_product_category':
				return '==' === $rule['operator'] ? is_product_category() : ! is_product_category();
				break;
			case 'woo_is_product_tag':
				return '==' === $rule['operator'] ? is_product_tag() : ! is_product_tag();
				break;
			case 'woo_is_product':
				return '==' === $rule['operator'] ? is_product() : ! is_product();
				break;
			case 'woo_is_cart':
				return '==' === $rule['operator'] ? is_cart() : ! is_cart();
				break;
			case 'woo_is_checkout':
				return '==' === $rule['operator'] ? is_checkout() : ! is_checkout();
				break;
			case 'woo_is_account_page':
				return '==' === $rule['operator'] ? is_account_page() : ! is_account_page();
				break;
		}
	}
}
